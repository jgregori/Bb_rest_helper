...
class Tests_Bb_rest_helper(unittest.TestCase):
    ...
    def test_collab_auth(self):
        self.config = Get_Config('./Bb_rest_helper/credentials/collab_config.json')
        self.url = self.config.get_url()
        self.key = self.config.get_key()
        self.secret = self.config.get_secret()
        self.auth = Auth_Helper(self.learn_url,self.key,self.secret)
    ...
    def test_Bb_POST_file(self):
        ...
        self.upload = self.reqs.Bb_POST_file(self.learn_url,self.learn_token,self.file_path)
        assert self.upload[0]

    @vcr.use_cassette('./Bb_rest_helper/vcr_tests/test_Bb_PATCH')
    def test_Bb_PATCH(self):
        self.announcement_id='_1420_1' #Get the id for the assment to patch from the response
        self.reqs = Bb_Requests()
    ...
    def test_Bb_DELETE(self):
        self.announcement_id='_1420_1' #Announcement id from emeasedemo, update if cassete changes.
        self.reqs = Bb_Requests()
        self.endpoint = f'/learn/api/public/v1/announcements/{self.announcement_id}'
        self.data = self.reqs.Bb_DELETE(self.learn_url,self.endpoint,self.learn_token)
    ...
    def test_check_course_id_true(self):
        ...
        check = self.utils.check_course_id(self.learn_url,self.learn_token,'AWPBL')
        assert check
    ...
    def test_check_course_id_false(self):
        ...
        check = self.utils.check_course_id(self.learn_url,self.learn_token,'111111')
        assertFalse(check)
    ...
    def test_learn_convert_external_id(self):
        ...
        data = self.utils.learn_convert_external_id(self.learn_url,self.learn_token, self.course_external_id)
        assert data
